...
const features = [
  {
    icon: <Plus size={18} className="text-[#00C2A8]" />,
  ...
  {
    icon: <MessageSquare size={18} className="text-[#00C2A8]" />,
  ...
  {
    icon: <Video size={18} className="text-[#00C2A8]" />,
    ...
    <div className="bg-[#FAF9F1]">
      ...
      <section className="py-20 border-b border-[#E2D8CC]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-[10px] uppercase tracking-[0.2em] text-[#D4AF57] font-bold mb-4">Collaborative Workspaces</p>
          <h1 className="text-4xl sm:text-5xl font-bold text-[#2C1F0E] mb-5">
            <span className="text-[#D4AF57]">Roundtables</span>
          </h1>
      ...
      <section className="py-16">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-xl font-bold text-[#2C1F0E]">Active Roundtables</h2>
            <Button size="sm">
            ...
            {roundtables.map((rt) => (
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-3">
                  <h3 className="text-[#2C1F0E] font-bold leading-snug">{rt.name}</h3>
                  {rt.status === 'Invitation Only' ? (
                    <Lock size={14} className="text-[#D4AF57] mt-0.5 shrink-0" />
                  ) : (
                    <div className="w-2 h-2 rounded-full bg-green-400 mt-1 shrink-0" />
                  )}
                </div>

                <p className="text-[#4A3828] text-sm leading-relaxed mb-4">{rt.description}</p>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-[#6E5E34] text-xs">
                    <Users size={12} />
                    {rt.members} members
                  </div>
                  {rt.status === 'Open' ? (
      ...
      {/* Features */}
      <section className="border-t border-[#E2D8CC] py-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <p className="text-[10px] uppercase tracking-[0.2em] text-[#D4AF57] font-bold mb-3">Platform Features</p>
            <h2 className="text-3xl font-bold text-[#2C1F0E]">Everything Your Roundtable Needs</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {features.map((feature) => (
              <div
                key={feature.title}
                className="bg-[#F3F0EA] border border-[#E2D8CC] rounded-xl p-5 hover:border-[#E2D8CC] transition-all"
              >
                <div className="mb-3">{feature.icon}</div>
                <h3 className="text-[#2C1F0E] font-bold text-sm mb-2">{feature.title}</h3>
                <p className="text-[#4A3828] text-xs leading-relaxed">{feature.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-[#E2D8CC] py-20">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl font-bold text-[#2C1F0E] mb-4">Launch Your Own Roundtable</h2>
