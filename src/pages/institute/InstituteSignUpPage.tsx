      ...
      <div className="min-h-screen py-16 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
            {TIERS.filter(t => t.id !== 'founding').map(t => (
              <button
                ...
                className="text-left bg-white/[0.025] border border-white/[0.07] hover:border-white/[0.14] hover:bg-white/[0.04] rounded-xl p-6 transition-all duration-200 cursor-pointer"
              >
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <div className="mt-3 flex items-baseline gap-1">
                      ...
                      {t.billing && <span className="text-sm text-white/40">{t.billing}</span>}
                    </div>
                  </div>
                </div>
                <ul className="space-y-2">
                  {t.perks.map(p => (
                    <li key={p} className="flex items-start gap-2 text-sm text-white/60">
                      <Check size={13} className="text-[#00FFE1] mt-0.5 shrink-0" />
                      {p}
                    </li>
                  ))}
                </ul>
                <div className="mt-5 text-sm font-semibold text-[#00FFE1]">
          ...
          {(() => {
            ...
            return (
              <button
                ...
                className="w-full text-left bg-[#c9a96e]/[0.04] border border-[#c9a96e]/20 hover:border-[#c9a96e]/40 rounded-xl p-6 transition-all duration-200 cursor-pointer"
              >
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                  <div className="flex-1">
                    <Badge variant="gold">Founding Member</Badge>
                    <p className="text-white font-bold mt-2 text-lg">
                      {f.price}<span className="text-white/40 text-sm font-normal">{f.billing}</span>
                    </p>
          ...
          <p className="text-center text-sm text-white/30 mt-8">
            Already a member?{' '}
            <Link to="/institute/login" className="text-[#00FFE1] hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    );
      ...
      <div className="w-full max-w-lg">
        <div className="flex items-center justify-center gap-2.5 mb-8">
          <div className="w-9 h-9 rounded-xl bg-[#00FFE1]/10 border border-[#00FFE1]/20 flex items-center justify-center">
            <BookOpen size={18} className="text-[#00FFE1]" />
          </div>
          <div>
        ...
        <div className="bg-white/[0.025] border border-white/[0.07] rounded-2xl p-8">
          ...
          {tier.stripePriceId && (
            <div className="bg-[#c9a96e]/[0.06] border border-[#c9a96e]/20 rounded-lg px-3 py-2.5 mb-5 text-sm text-[#c9a96e]/80">
              After creating your account you'll be redirected to Stripe to complete payment.
            </div>
          )}

          <form onSubmit={handleSubmit} className="space-y-4">
            <Field label="Full name" icon={<User size={15} className="text-white/25" />}>
              <input
                required
                value={displayName}
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Organization" icon={<Building2 size={15} className="text-white/25" />}>
                <input
                  value={organization}
              ...
              <Field label="Job title" icon={<Briefcase size={15} className="text-white/25" />}>
                <input
                  value={jobTitle}
            ...
            <Field label="Email address" icon={<Mail size={15} className="text-white/25" />}>
              <input
                type="email"
                required
                value={email}
            ...
            <Field label="Password" icon={<Lock size={15} className="text-white/25" />} rightSlot={
              <button type="button" onClick={() => setShowPass(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/25 hover:text-white/50 transition-colors">
            ...
            <Field label="Confirm password" icon={<Lock size={15} className="text-white/25" />}>
              <input
                type={showPass ? 'text' : 'password'}
                required
...
const inputClass =
  'w-full bg-white/[0.04] border border-white/[0.08] rounded-lg pl-9 pr-4 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-[#00FFE1]/40 focus:bg-white/[0.06] transition-colors';
