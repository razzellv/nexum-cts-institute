...
const statusStyles: Record<string, string> = {
  ...
  Draft: 'bg-[#D4AF57]/10 border border-[#c9a96e]/30 text-[#D4AF57]',
  'In Development': 'bg-[#00C2A8]/10 border border-[#00C2A8]/30 text-[#00C2A8]',
  Planned: 'bg-white/5 border border-white/15 text-[#6E5E34]',
}
  ...
  return (
    <div className="bg-[#FAF9F1] text-[#2C1F0E]">
      ...
      <section className="py-16 px-4 sm:px-6 max-w-7xl mx-auto">
        <p className="text-[10px] uppercase tracking-[0.2em] text-[#D4AF57] font-bold mb-3">Standards</p>
        <h1 className="text-3xl md:text-4xl font-bold text-[#2C1F0E] mb-4">CTS™ Standards Library</h1>
        <p className="text-[#4A3828] text-sm leading-relaxed max-w-2xl mb-8">
          ...
          {filters.map((f) => (
            <button
              ...
              className={cn(
                ...
                activeFilter === f
                  ? 'bg-[#00C2A8] text-black font-bold'
                  : 'bg-[#EDE8E0] border border-[#E2D8CC] text-[#4A3828] hover:text-[#00C2A8] hover:border-white/20'
              )}
            >
              {f}
            </button>
      ...
      <section className="px-4 sm:px-6 max-w-7xl mx-auto pb-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filtered.map((std) => (
            <div
              key={std.id}
              className="bg-[#F3F0EA] border border-[#E2D8CC] rounded-xl p-6 hover:border-[#E2D8CC] hover:bg-[#EDE8E0] transition-all flex flex-col"
            >
              {/* Top badges */}
              <div className="flex items-center gap-2 mb-4 flex-wrap">
                <span className="text-xs px-2.5 py-0.5 rounded-full border border-[#00C2A8]/30 text-[#00C2A8] font-mono font-bold">
                  {std.id}
                </span>
                <span className={cn('text-xs px-2.5 py-0.5 rounded-full font-medium', statusStyles[std.status])}>
                  {std.status}
                </span>
                <span className="text-xs text-[#7A6A55] ml-auto font-mono">{std.version}</span>
              </div>

              <h3 className="text-xl font-bold text-[#2C1F0E] mb-2">{std.title}</h3>
              <p className="text-[#4A3828] text-sm leading-relaxed mb-4 flex-1">{std.description}</p>

              {std.related && (
                <p className="text-xs text-[#6E5E34] mb-4">Related: {std.related}</p>
              )}

              <div className="border-t border-[#E2D8CC] pt-4 flex flex-wrap gap-2">
                {std.actions.includes('preview') && (
                  <button className="px-4 py-2 rounded-lg border border-[#00C2A8]/40 text-[#00C2A8] hover:bg-[#00C2A8]/10 transition-all text-xs font-medium">
                    Free Preview — first 10 pages
                  </button>
                )}
                {std.actions.includes('purchase') && (
                  <button className="px-4 py-2 rounded-lg bg-[#00C2A8] text-black font-bold hover:bg-[#00C2A8]/90 transition-all text-xs">
                    ...
                    {waitlistStd === std.id ? (
                      <div className="flex gap-2">
                        <input
                          ...
                          className="px-3 py-2 rounded-lg bg-[#EDE8E0] border border-white/10 text-[#2C1F0E] text-xs placeholder:text-[#7A6A55] outline-none focus:border-[#00C2A8]/40 w-40"
                        />
                        <button className="px-3 py-2 rounded-lg bg-[#00C2A8] text-black font-bold text-xs hover:bg-[#00C2A8]/90 transition-all">
                          Submit
                        </button>
                      </div>
                    ) : (
                      <button
                        ...
                        className="px-4 py-2 rounded-lg border border-white/20 text-[#4A3828] hover:text-[#00C2A8] hover:border-white/40 transition-all text-xs font-medium"
                      >
                        Join Waitlist — Free
                      </button>
                    )}
                ...
                {std.actions.includes('notify') && (
                  <button className="px-4 py-2 rounded-lg border border-white/15 text-[#6E5E34] hover:text-[#4A3828] hover:border-white/25 transition-all text-xs font-medium">
      ...
      {/* Bottom info cards */}
      <section className="px-4 sm:px-6 max-w-7xl mx-auto pb-16">
        <h2 className="text-2xl font-bold text-[#2C1F0E] mb-6">What Makes a CTS™ Standard?</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={title} className="bg-white/[0.02] border border-[#E2D8CC] rounded-xl p-5">
              <h3 className="font-bold text-[#2C1F0E] mb-2">{title}</h3>
              <p className="text-[#4A3828] text-sm leading-relaxed">{body}</p>
            </div>
          ))}
        </div>
      </section>
